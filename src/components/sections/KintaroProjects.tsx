...
export default function KintaroProjects() {
    // Map portfolioData.projects to ProjectItem format
    const projects: ProjectItem[] = portfolioData.projects.map((p: any) => ({
        ...
        image: (p as any).image || (p as any).thumbnail || "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?q=80&w=2000&auto=format&fit=crop",
        demo: p.demoUrl,
        repo: p.repoUrl,
        stack: p.techStack
    }));
...
const ProjectCard = React.memo(function ProjectCard({ project, onClick }: { project: ProjectItem; onClick?: () => void }) {
    return (
        <BlurReveal>
            <div
                onClick={onClick}
                className="group relative w-full xl:w-[45vw] aspect-[4/3] shrink-0 xl:mx-6 perspective-[1000px] cursor-pointer"
            >
